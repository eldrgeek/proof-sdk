...
const serverView = (page, slug) => page.evaluate(async s => {
  const r = await fetch(`/api/documents/${s}/line-marks`, { credentials: 'same-origin', headers: { 'X-Proof-Client-Version': '0.34.0', 'X-Proof-Client-Build': 'test', 'X-Proof-Client-Protocol': '3' } });
  return { status: r.status, body: await r.json() };
}, slug);

const pagePost = (page, url, body) => page.evaluate(async ({ u, b }) => {
  const r = await fetch(u, { method: 'POST', credentials: 'same-origin', headers: { 'Content-Type': 'application/json', 'X-Proof-Client-Version': '0.34.0', 'X-Proof-Client-Build': 'test', 'X-Proof-Client-Protocol': '3' }, body: JSON.stringify(b) });
...
async function run(browser, style) {
  ...
  try {
    ...
    await check(`${tag}: a guest reads and comments, cannot edit, and is told "Sign in"`, async () => {
      ...
      const md = await fetch(`${base}/api/documents/${slug}`, { headers: { 'X-Proof-Client-Version': '0.34.0', 'X-Proof-Client-Build': 'test', 'X-Proof-Client-Protocol': '3' } }).then(r => r.json());
