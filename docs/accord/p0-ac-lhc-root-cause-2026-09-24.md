...
## Outcome (added 2026-09-24, 17:10 UTC)

- **Built.** Codex built the design in two rounds: `bb8bdb8` (the four parts) and `39bc0c6` (the review's additions: restores never overwrite a newer write; after 3 restores in 5 minutes a connection is closed and asked to reload; clients older than 0.32.0 are refused for live editing, with the client version signed into the session token; resolved proposals are pruned after 24 hours; a deletion replayed from a tab's queue is restored). The reviewer fixed the check's hidden variant in `ffbab04`, because headless Chromium cannot hide a page.
- **Verified before the deploy.** The restart check failed on the unfixed d15df82 at iteration 1, both navigating away and hiding. On `ffbab04` it passed 40 of 40, including 20 where a page on 0913728 met the new server. The replay of the real incident kept both suggestions 4 of 4 times. `npm test` passed (38 groups). All 35 existing browser checks and line-marks in the proof style passed. The acceptance harness passed 26 of 26. TypeScript reported 475 errors, the same as before.
- **Deployed** at 16:58 UTC (backup `proof-share-20260924T165757Z-pre-usability-w2b.db`). The live check repeated the incident: an old-client page open during the deploy was refused, the same tab loaded the new client, and both pending suggestions survived. Accept and Undo wrote `suggestion.accepted` and `suggestion.reopened`. Across every document, the 198 pending suggestions before the deploy were all still pending after it.
- **Known limit.** A page loaded before 16:58 runs the old build. It shows "Offline – reconnecting" until it is reloaded, and it keeps asking for a session every few seconds, which the server refuses. Nothing typed in it is lost.
