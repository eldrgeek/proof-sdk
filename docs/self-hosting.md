...
SOMA tokens stay in the browser. The server verifies them remotely and stores only a hashed Accord session identifier and a dated role result. After 24 hours, administrator authority expires until the browser supplies a fresh token for verification. The browser schedules that check at the server's lease deadline. A failed check cannot extend administrator authority. Successful daily checks slide the existing 180-day Accord session. A removed administrator may remain an ordinary member if they have an active library membership.
