...
export const MARKS_LEGEND: ReadonlyArray<{ swatch: 'here' | 'need' | 'settled' | 'change' | 'status'; label: string; means: string }> = [
  { swatch: 'here', label: 'Blue bar', means: 'The selected passage. S places the caret here.' },
  ...
  { swatch: 'status', label: 'Seen', means: 'A stored reading record from an earlier review. Scrolling no longer writes one.' },
