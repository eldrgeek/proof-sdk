...
async function call(url: string, headers: Headers, body?: unknown, method = body === undefined ? 'GET' : 'POST') {
  const response = await fetch(base + url, { method, headers: { 'Content-Type': 'application/json', 'X-Proof-Client-Version': '0.31.0', 'X-Proof-Client-Build': 'test', 'X-Proof-Client-Protocol': '3', ...headers }, body: body === undefined ? undefined : JSON.stringify(body) });
