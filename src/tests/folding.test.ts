...
try {
  ...
  await test('section Issue count: the same Issues as the top bar, restricted to the section (review marks by position)', () => {
    ...
    assert.deepEqual(count(GOALS), { total: 2, lines: 1, reviewMarks: 1 });
    assert.deepEqual(count(DETAIL), { total: 1, lines: 0, reviewMarks: 1 });
    assert.deepEqual(count(PLAN), { total: 1, lines: 1, reviewMarks: 0 });
    assert.deepEqual(count(TITLE), { total: 3, lines: 2, reviewMarks: 1 }, 'a parent counts its sub-sections');
